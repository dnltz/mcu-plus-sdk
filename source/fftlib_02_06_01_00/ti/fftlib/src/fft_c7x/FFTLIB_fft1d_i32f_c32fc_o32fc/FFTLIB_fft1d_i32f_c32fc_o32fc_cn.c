                                  ...
                                  FFTLIB_bufParams1D_t *bufParamsY)
{
   ...
   /* #endif */
   {
      ...
      FFTLIB_fft1d_i32fc_c32fc_o32fc_cn (
          pX, &bufParamsX_FFT, pW, &bufParamsX_FFT, pXFFT, &bufParamsX_FFT);
      ...
      temp1Real = var1Real * pSf[2 * i + numPoints] +
                  var1Imag * pSf[2 * i + numPoints + 1];
      temp1Imag = var1Real * pSf[2 * i + numPoints + 1] -
      ...
      for (i = 0; i < numPoints >> 1; i += 1) {
         ...
         // B*X'[N-k]
         if (!i) {
            temp1Real = pXFFT[0] * pSf[2 * numPoints - 2 * i - 2] +
                        pXFFT[1] * pSf[2 * numPoints - 2 * i - 2 + 1];
            temp1Imag = pXFFT[0] * pSf[2 * numPoints - 2 * i - 2] -
                        pXFFT[1] * pSf[2 * numPoints - 2 * i - 2 + 1];
         }
         else {
            /* temp1Real = */
             ...
             * numPoints]; */
            temp1Real = var1Real * pSf[2 * numPoints - 2 * i - 2] +
                        var1Imag * pSf[2 * numPoints - 2 * i - 2 + 1];
            temp1Imag = var1Real * pSf[2 * numPoints - 2 * i - 2 + 1] -
                        var1Imag * pSf[2 * numPoints - 2 * i - 2];
         }
