...
int32_t DSPLIB_fir_getHandleSize(
...
DSPLIB_STATUS DSPLIB_fir_init_checkParams(
              ...
              const DSPLIB_fir_InitArgs *pKerInitArgs)
{
   ...
   if(handle == NULL) {
      ...
      if (bufParamsIn->data_type == DSPLIB_INT16) {
         status = DSPLIB_fir_getSizes<int16_t>(pKerInitArgs,
                                                                    &bufParamsInLocal,
                                                                    &bufParamsFilterLocal,
                                                                    &bufParamsOutLocal);
      } else if (bufParamsIn->data_type == DSPLIB_INT32) {
         status = DSPLIB_fir_getSizes<int32_t>(pKerInitArgs,
                                                                    &bufParamsInLocal,
                                                                    &bufParamsFilterLocal,
                                                                    &bufParamsOutLocal);
      } else if (bufParamsIn->data_type == DSPLIB_INT8) {

         status = DSPLIB_fir_getSizes<int8_t>(pKerInitArgs,
                                                                   &bufParamsInLocal,
                                                                   &bufParamsFilterLocal,
                                                                   &bufParamsOutLocal);
      } else if (bufParamsIn->data_type == DSPLIB_FLOAT32) {
         status = DSPLIB_fir_getSizes<float>(pKerInitArgs,
                                                                    &bufParamsInLocal,
                                                                    &bufParamsFilterLocal,
                                                                    &bufParamsOutLocal);
       } else {
...
DSPLIB_STATUS DSPLIB_fir_getSizes(
   const DSPLIB_fir_InitArgs * pKerInitArgs,
...
DSPLIB_STATUS DSPLIB_fir_exec_checkParams(
  DSPLIB_kernelHandle handle,
              ...
              DSPLIB_fir_InitArgs *pKerInitArgs)
{
   ...
   {
      status = DSPLIB_fir_init_cn(handle,
                                                         bufParamsIn,
                                                         bufParamsFilter,
                                                         bufParamsOut,
                                                         pKerInitArgs) ;
