...
int32_t DSPLIB_add_getHandleSize(
        DSPLIB_add_InitArgs *pKerInitArgs)
{
  int32_t privBufSize = sizeof(DSPLIB_add_PrivArgs);
  return privBufSize ;
}

DSPLIB_STATUS DSPLIB_add_init_checkParams(
...
DSPLIB_STATUS DSPLIB_add_exec_checkParams(
  DSPLIB_kernelHandle handle,
...
#if DSPLIB_DEBUGPRINT
       printf("DSPLIB_DEBUGPRINT DSPLIB_add_init pKerPrivArgs->blockSize %d bufParamsIn->data_type %d\n", pKerPrivArgs->blockSize, bufParamsIn->data_type);
#endif
