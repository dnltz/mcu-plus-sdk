...
/**
  ******************************************************************************
  *
  *  @brief    File to hold buffer parameter related info for DSPLIB
  *
...
/*  being addressed                                                 */
/* ---------------------------------------------------------------- */  
// MACRO for determining signedness of DSPLIB data types; fragile in that it depends on ordering of DSPLIB_data_type_e
...
typedef struct
{
    ...
    int32_t  stride_2; /*!< \brief  Stride in bytes; Uninitialized when dimension = {DSPLIB_2D} */

    uint32_t dim_3;  /*!< \brief  Uninitialized when dimension = {DSPLIB_2D, DSPLIB_3D} */
    int32_t  stride_3; /*!< \brief  Stride in bytes; Uninitialized when dimension = {DSPLIB_2D, DSPLIB_3D} */

    uint32_t dim_4;  /*!< \brief  Uninitialized when dimension = {DSPLIB_2D, DSPLIB_3D, DSPLIB_4D} */
    int32_t  stride_4; /*!< \brief  Stride in bytes; Uninitialized when dimension = {DSPLIB_2D, DSPLIB_3D, DSPLIB_4D} */

    uint32_t dim_5;  /*!< \brief  Uninitialized when dimension = {DSPLIB_2D, DSPLIB_3D, DSPLIB_4D, DSPLIB_5D} */
    int32_t  stride_5; /*!< \brief  Stride in bytes; Uninitialized when dimension = {DSPLIB_2D, DSPLIB_3D, DSPLIB_4D, DSPLIB_5D} */
...
/*! \brief Inline function returns number of bytes per element given a type of \ref DSPLIB_data_type_e. */
static inline int32_t DSPLIB_sizeof(uint32_t type)
{

   int32_t size = 1;

   if((type == (uint32_t)DSPLIB_INT8)  ||
      (type == (uint32_t)DSPLIB_UINT8))
   {
      size = 1;
   }
   else if((type == (uint32_t)DSPLIB_INT16)  ||
	   (type == (uint32_t)DSPLIB_UINT16) ||
   ...
   else if((type == (uint32_t)DSPLIB_INT32)  ||
	   (type == (uint32_t)DSPLIB_UINT32) ||
	   (type == (uint32_t)DSPLIB_FLOAT32))
   {
      size = 4;
   }
   else if((type == (uint32_t)DSPLIB_INT64)  ||
	   (type == (uint32_t)DSPLIB_UINT64) ||
	   (type == (uint32_t)DSPLIB_FLOAT64))
   {
      size = 8;
   }
   else if((type == (uint32_t)DSPLIB_INT128)  ||
	   (type == (uint32_t)DSPLIB_UINT128))
   {
      size = 16;
   }
   else {
