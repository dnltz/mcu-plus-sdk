...
{
  int32_t privBufSize = sizeof(MATHLIB_div_PrivArgs);
  return privBufSize ;
}
...
//   MATHLIB_bufParams2D_t bufParamsInLocal, bufParamsOutLocal, bufParamsFilterVarLocal;
   ...
   printf("Enter MATHLIB_div_init_checkParams\n");
#endif
   if(handle == NULL) {
...
#if MATHLIB_DEBUGPRINT
   printf("Enter MATHLIB_div_exec_checkParams\n");
#endif
...
{
   MATHLIB_STATUS status = MATHLIB_SUCCESS;
   MATHLIB_div_PrivArgs *pKerPrivArgs =
   (MATHLIB_div_PrivArgs*) handle ;
   
#if MATHLIB_DEBUGPRINT
       printf("MATHLIB_DEBUGPRINT Enter MATHLIB_div_init\n");
#endif
...
#if MATHLIB_DEBUGPRINT
       printf("MATHLIB_DEBUGPRINT MATHLIB_div_init pKerPrivArgs->blockSize %d bufParamsIn->data_type %d\n", pKerPrivArgs->blockSize, bufParamsIn->data_type);
#endif
...
#endif
         pKerPrivArgs->execute =
            MATHLIB_div_exec_ci<float, int32_t, MATHLIB_FLOAT32>;
      } else {
         pKerPrivArgs->execute =
            MATHLIB_div_exec_ci<double, int64_t, MATHLIB_FLOAT64>;
      }
   }
#if MATHLIB_DEBUGPRINT
...
#if MATHLIB_DEBUGPRINT
       printf("MATHLIB_DEBUGPRINT Enter MATHLIB_div_exec\n");
#endif
