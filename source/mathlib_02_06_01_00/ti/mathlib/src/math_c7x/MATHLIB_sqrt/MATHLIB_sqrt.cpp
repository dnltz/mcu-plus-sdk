...
{
  int32_t privBufSize = sizeof(MATHLIB_sqrt_PrivArgs);
  return privBufSize ;
}
...
//   MATHLIB_bufParams2D_t bufParamsInLocal, bufParamsOutLocal, bufParamsFilterVarLocal;
   ...
   printf("Enter MATHLIB_sqrt_init_checkParams\n");
#endif
   if(handle == NULL) {
...
#if MATHLIB_DEBUGPRINT
   printf("Enter MATHLIB_sqrt_exec_checkParams\n");
#endif
...
{
   MATHLIB_STATUS status = MATHLIB_SUCCESS;
   MATHLIB_sqrt_PrivArgs *pKerPrivArgs =
   (MATHLIB_sqrt_PrivArgs*) handle ;
   
#if MATHLIB_DEBUGPRINT
       printf("MATHLIB_DEBUGPRINT Enter MATHLIB_sqrt_init\n");
#endif
...
#if MATHLIB_DEBUGPRINT
       printf("MATHLIB_DEBUGPRINT MATHLIB_sqrt_init pKerPrivArgs->blockSize %d bufParamsIn->data_type %d\n", pKerPrivArgs->blockSize, bufParamsIn->data_type);
#endif
...
#endif
         pKerPrivArgs->execute =
            MATHLIB_sqrt_exec_ci<float, int32_t, MATHLIB_FLOAT32>;
      } else {
         pKerPrivArgs->execute =
            MATHLIB_sqrt_exec_ci<double, int64_t, MATHLIB_FLOAT64>;
      }
   }
#if MATHLIB_DEBUGPRINT
...
#if MATHLIB_DEBUGPRINT
       printf("MATHLIB_DEBUGPRINT Enter MATHLIB_sqrt_exec\n");
#endif
